...
############################
# Runtime stage – keep Alpine Metabase
############################
...
# 1️⃣ tools needed to unpack the .deb (dpkg brings dpkg-deb)
RUN apk add --no-cache wget ca-certificates dpkg xz libstdc++

# 2️⃣ download + overlay modern glibc
RUN set -eux; \
    ...
    mv /tmp/glibc/lib/x86_64-linux-gnu /usr/glibc-compat/lib; \
...
# 3️⃣ MotherDuck (DuckDB) Metabase driver
RUN mkdir -p /plugins && \
